...
runID    =  'onephs';            % run identifier
outdir   = '../out/';            % output directory
srcdir   = '../src/';            % source directory
restart  =  0;                   % restart from file (0: new run; <1: restart from last; >1: restart from specified frame)
nop      =  10;                  % output frame plotted/saved every 'nop' time steps
plot_op  =  1;                   % switch on to live plot results
plot_crc =  1;                   % switch on particle and melt halo circles on plots
...
etap     =  1e6;                 % particle-melt viscosity contrast (leave as is) [Pas]
rm_fact  =  1;                   % relative melt halo size (= 1: mean particle distance)    
